...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Initializing the state parameters at t=t+1
    for (int t = 1; t < N; t++) {
    ...
    // Initializing the state parameters at t=t
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set the initial variable values according to the received state
  double x = state[0];
  double y = state[1];
  double psi = state[2];
  double v = state[3];
  ...
  //Updating the constraints bounds according to the received state
  ...
  // Getting the actuations and the predicted waypoints to send them back to the simulator
